...
| 2026-07-27 | ARM64 NEON 128-Token Generation (`tg128`) vs `llama.cpp` CPU NEON (`stories15M-q4_0.gguf`) | ARM64 NEON / Apple M1 (Virtual), macOS 14, GitHub Actions `macos-14` runner | `asmllm` hand-written NEON assembly vs `llama.cpp` pure CPU NEON (`commit 961e4b2`) | **1T:** `asmllm` **236.53 t/s** vs `llama.cpp` **1538.85 t/s** (asmllm 6.5x slower)<br>**2T:** `asmllm` **263.45 t/s** vs `llama.cpp` **1804.72 t/s** (asmllm 6.8x slower)<br>**4T/8T:** unreliable — llama.cpp collapses to 4.55/0.90 t/s | Verified / CI `arm64-apple-silicon-benchmarks.zip` (commit 3b09fa0) | CI run for M6 commit. Still an honest loss against llama.cpp's mature NEON intrinsics on ARM64. 4T/8T remain unreliable on GitHub Actions virtualization. |
