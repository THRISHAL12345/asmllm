...
| 2026-07-10 | ARM64 NEON 128-Token Generation (`tg128`) vs `llama.cpp` CPU NEON (`stories15M-q4_0.gguf`) | ARM64 NEON / Apple M1 (Virtual), macOS 14, GitHub Actions `macos-14` runner | `asmllm` hand-written NEON assembly vs `llama.cpp` pure CPU NEON (`commit 961e4b2`, `-DGGML_BLAS=OFF -DGGML_METAL=OFF`) | **1T:** `asmllm` **288.43 t/s** vs `llama.cpp` **1933.47 t/s** (asmllm 6.7x slower)<br>**2T:** `asmllm` **305.12 t/s** vs `llama.cpp` **1821.55 t/s** (asmllm 6.0x slower)<br>**4T/8T:** unreliable — llama.cpp collapses to 4.41/0.71 t/s, likely runner oversubscription (see notes) | Verified / `bench/results/2026-07-10-arm64-neon/raw_bench.log` | M5 ARM64 port: honest loss on 1T/2T (the only trustworthy data points). 4T/8T data is unreliable on this virtualized runner — physical core count unknown (hw.ncpu not yet captured). llama.cpp's mature NEON intrinsics path vs asmllm's first-generation hand-written GAS assembly. |
...
| 2026-07-27 | `matmul_q4_mt` (Adaptive Spinlock Threadpool Rewrite) | x86-64 AVX2 / Intel Core 5 210H (12 logical cores, Win11) | 1, 2, 4, 8 thread scaling | **1T:** 1.248 ms (**26.88 GFLOPS**)<br>**4T:** 0.300 ms (**112.02 GFLOPS**, 4.17x)<br>**8T:** 0.380 ms (**88.32 GFLOPS**) | Verified / `bench/results/2026-07-27-mt_throughput/raw_bench.log` | Threadpool rewritten to use C11/Win32 lock-free atomic adaptive spin-waits (YieldProcessor/mm_pause) to eliminate OS kernel context switch latency on dispatch. Achieved super-linear scaling (4.17x) on 4 threads, breaking the 110 GFLOPS barrier. Note: Token generation absolute t/s dropped during this run due to host CPU thermal/power throttling, but relative scaling is rock solid. |
| 2026-07-27 | ARM64 NEON 128-Token Generation (`tg128`) vs `llama.cpp` CPU NEON (`stories15M-q4_0.gguf`) | ARM64 NEON / Apple M1 (Virtual), macOS 14, GitHub Actions `macos-14` runner | `asmllm` hand-written NEON assembly vs `llama.cpp` pure CPU NEON (`commit 961e4b2`) | **1T:** `asmllm` **252.55 t/s** vs `llama.cpp` **1971.51 t/s** (asmllm 7.8x slower)<br>**2T:** `asmllm` **251.17 t/s** vs `llama.cpp` **2770.69 t/s** (asmllm 11.0x slower)<br>**4T/8T:** unreliable — llama.cpp collapses to 4.52/0.90 t/s | Verified / CI `arm64-apple-silicon-benchmarks` (commit 3b09fa0) | CI run for M6 commit. Still an honest loss against llama.cpp's mature NEON intrinsics on ARM64. 4T/8T remain unreliable on GitHub Actions virtualization. |
